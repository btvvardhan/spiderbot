...
def _reset_idx(self, env_ids: torch.Tensor | None):
    ...
    if not hasattr(self, 'curriculum_level'):
        self.curriculum_level = 0
        self.episode_returns_history = []  # Track actual returns, not components
    
    # Update curriculum based on recent TOTAL rewards
    if len(self.episode_returns_history) > 20:  # After 20 episodes
        recent_avg_return = sum(self.episode_returns_history[-20:]) / 20
        
        # Adjusted thresholds based on your actual reward scale
        if recent_avg_return > 100 and self.curriculum_level == 0:
            self.curriculum_level = 1
            print(f"🎯 CURRICULUM LEVEL 1: Adding speed variation!")
        elif recent_avg_return > 103 and self.curriculum_level == 1:
            self.curriculum_level = 2
            print(f"🎯 CURRICULUM LEVEL 2: Adding turning!")
        elif recent_avg_return > 105 and self.curriculum_level == 2:
            self.curriculum_level = 3
            print(f"🎯 CURRICULUM LEVEL 3: Full complexity with lateral!")
        elif recent_avg_return < 95 and self.curriculum_level > 0:
            self.curriculum_level = max(0, self.curriculum_level - 1)
            print(f"📉 Curriculum dropped to level {self.curriculum_level}")
    
    # Set commands based on curriculum level
    ...
    # Reset robot state (keep your existing code)
    ...
    # Log curriculum info with CORRECT total
    extras["Curriculum/level"] = float(self.curriculum_level)
    
    # Only track returns for fully completed episodes (not resets from death)
    if hasattr(self, 'episode_return'):
        mean_return = self.episode_return[env_ids].mean().item()
        if mean_return > 0:  # Valid episode
            self.episode_returns_history.append(mean_return)
            if len(self.episode_returns_history) > 100:
                self.episode_returns_history.pop(0)
            extras["Curriculum/avg_return"] = mean_return
